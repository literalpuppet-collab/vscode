...
function SetDefaults() {
    ...
    UserID="";SetObjectValueID("userid",UserID);
    LicenceID="";SetObjectValueID("licenceid",LicenceID);
...
function onRegistrationInputChanged() {
    UserID=UserIDInput.value;
	SetObjCookie(UserIDInput,"userid");

    LicenceID=LicenceIDInput.value;
	SetObjCookie(LicenceUserIDInput,"licenceid");
}
...
function validateEndPointObjectChanges() {
	...
	for(var i=0;i<NumOfStreams;i++) {
		...
		var authError = false;
		if(epah.length>0){
			authError = (epah.search(/^[\-a-fA-F0-9]{36}$/)==-1 ? true : false);
		}
...
function DoUpdate(mode) {
	if(mode == null) {
		...
		output += encodeURIComponent("userid="+UserID)+NL;
		output += encodeURIComponent("licenceid="+LicenceID)+NL;
		...
		config = "";
		NL = "</b><br>";
		config += "<fieldset style=\"text-align:center;width:inherit;\"><legend class=\"titlespan\"><b>Registration</b></legend>";
		config += "Licence ID: <b>"+LicenceID+"</b><br>";
		config += "User ID: <b>"+UserID+"</b><br></fieldset><br>";
...
function MultiPointSpanUpdate() {
	...
	for(var i=0;i<NumOfStreams;i++) {
		...
		str+="Stream Authhash</td><td><input maxlength=\"36\" size=\"36\" name=\""+naid+"\" id=\""+naid+"\"/><br/>";
...
function DoInit() {
	...
	UserID = GetObjCookie(UserIDInput = register("userid",onRegistrationInputChanged));
	LicenceID = GetObjCookie(LicenceIDInput = register("licenceid",onRegistrationInputChanged));
	...
	xmlhttp.onreadystatechange=function(){
		if(xmlhttp.readyState==4){
			if(xmlhttp.status==200){
                ...
                $('results').innerHTML = "code is "+xmlhttp.status;
		}
	}
}
